...
| Carte | Note |
|---|---|
| `536-la-suite-e2e-a-sa-propre-base-clonee-avant-chaque-passage` | La suite tourne dans la base où l'on travaille à la main, et rien ne la purge jamais : 155 Mo après une journée, dont 94 % de résidus, et 25 % de lenteur que rien ne signale. Une base dédiée, clonée depuis un gabarit en moins d'une seconde — on clone, on ne recrée pas, parce que rejouer les migrations est un coût qui grandit |
...
| `543-quatre-fragments-d-administration-sans-controle-d-acces` | Quatre handlers `GET` de l'administration — barre latérale du Calendrier, détail d'une journée, cartes de poules, vivier non affecté — n'appellent pas `require_admin_access`, et aucune couche du routeur ne le fait pour eux. Ni accès anonyme, ni inter-espaces, ni écriture : un **membre simple de l'espace** lit quatre vues d'administration. La carte 416 avait fermé le même trou sur les treize routes de **mutation** ; les fragments de lecture n'y figuraient pas |
| `542-extraire-la-campagne-de-presence-en-bc` | Décision d'architecture, à trancher **après la vague 4 de E16**. Rien ne dépend du code des présences — c'est une feuille — mais il n'importe pas, il **écrit** : appariements, suppressions, `PairingCreated`. Couper entre la campagne et le tirage ne supprime pas le couplage, il le retourne : ou `presences` écrit des appariements, ou `competitions` mute une campagne. Le critère qui décide est celui que l'écran révélera — l'onglet composera-t-il des widgets de deux BCs ou d'un seul |
...
| `499-la-correction-etait-jugee-avant-que-les-equipes-aient-bouge` ✅ | Le test de correction ouvrait le récapitulatif dès le rapport publié, alors que l'entrée des équipes en phase d'amélioration passe par un listener cross-BC : le verdict tombait sur un camp qui n'avait pas encore bougé, et le bouton était rendu désactivé. Troisième occurrence de la même famille en une journée, après le câblage htmx et la projection du bus — et quatrième copie du même helper d'attente |
| `496-deux-tests-lisent-la-projection-avant-que-le-bus-ait-fini` ✅ | Deux tests de `test_haine_journalier` lisaient `players_proj` au retour de la publication, alors que les impacts joueur transitent par le bus d'app events. L'un tombait par intermittence — vert sur une suite en 6 min, rouge sur la même en 8 — et accusait le produit ; l'autre, qui vérifie qu'il ne s'est rien passé, était vert **même si rien n'arrivait jamais**. Une seconde famille de course, distincte du câblage htmx |
