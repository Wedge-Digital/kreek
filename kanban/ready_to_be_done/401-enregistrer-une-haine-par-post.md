...
```rust
pub struct KeywordDto { pub uid: String, pub label: String }
...
| uid absent du catalogue | le **use case**, via `exists` | `UnknownKeyword(uid)` → 422 |
...
- [ ] Tests unitaires :
  ...
  - [ ] chemin nominal → action enregistrée, Haine dans l'événement
