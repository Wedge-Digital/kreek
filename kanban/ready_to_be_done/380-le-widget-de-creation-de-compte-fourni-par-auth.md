...
- [ ] Route `COACH_CREATION_WIDGET`, GET et POST
- [ ] Gabarit : Pseudo, Email, bouton — **pas de sélecteur de profil**,
      `SpaceProfile` est un concept de `spaces`
- [ ] Erreurs de `create_account_without_password` rendues **dans le fragment**,
      jamais remontées à l'appelant
- [ ] `HX-Trigger: accountCreated` avec `coach_id` et `name`, commenté comme
      contrat non vérifié
- [ ] Pré-remplissage par `CoachPrefill`
- [ ] Feuille servie par `auth`, absente de `FEUILLES_APP`, n'utilisant que des
      tokens globaux
- [ ] `auth` n'importe rien de `spaces` — l'axe 9 de `check-arch` le vérifie
- [ ] Tests du harnais handler : succès, pseudo pris, email pris — le fragment
      rend l'erreur et **ne pose pas** `HX-Trigger`
