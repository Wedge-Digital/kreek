...
impl ResendMailService {
    ...
    #[cfg(test)]
    fn with_base_url(base_url: String, api_key: String, sender_email: String, sender_name: String) -> Self {
        ResendMailService {
            client:       Client::new(),
...
mod tests {
    ...
    async fn send_success_renvoie_ok() {
        ...
        server.mock("POST", "/emails")
            .with_status(200)
            .with_body(r#"{"id":"msg-001"}"#)
            .create_async().await;

        let result = service(server.url())
            .send(vec!["dest@example.com".into()], "Sujet".into(), "<p>Test</p>".into())
            .await;
    ...
    async fn send_api_rejected_renvoie_erreur_avec_status() {
        ...
        assert!(matches!(result, Err(EmailError::ApiRejected { status: 422, .. })));
    }
    ...
    async fn send_transmet_le_bon_header_auth() {
        let mut server = Server::new_async().await;
        let mock = server.mock("POST", "/emails")
    ...
    async fn integration_envoi_email_reel() {
        let api_key = std::env::var("RESEND_API_KEY")
            .expect("RESEND_API_KEY doit être défini pour ce test");
        let to = std::env::var("RESEND_TEST_TO")
            .expect("RESEND_TEST_TO doit être défini pour ce test");

        let service = ResendMailService::new(
            api_key,
            "mailer@example.com".into(),
            "Kreek".into(),
        );
    ...
    async fn send_transmet_le_bon_payload() {
        ...
        service(server.url())
            .send(vec!["dest@example.com".into()], "Réinitialisation".into(), "<p>Lien</p>".into())
            .await
            .unwrap();
