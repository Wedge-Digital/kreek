...
async fn get_step(space_id: String, mr_id: String, side: TeamSide, state: AppState) -> Response {
    let state_opt = match state
        .match_report
        .match_report_repo
        .find_by_id(&mr_id)
        .await
    ...
    let (home_id, away_id) = match state_opt {
        Some(MatchReportState::PreMatch(pm)) => {
            (pm.home_team_id.to_string(), pm.away_team_id.to_string())
        }
        Some(MatchReportState::ReadyToPublish(rtp)) => {
            (rtp.home_team_id.to_string(), rtp.away_team_id.to_string())
        }
    ...
    let team_id = match side {
        TeamSide::Home => home_id.clone(),
        TeamSide::Away => away_id.clone(),
    };
    ...
    let team_side_label = match side {
        TeamSide::Home => "home",
        TeamSide::Away => "away",
    };
    ...
    let (next_url, next_label, next_disabled) = match side {
        TeamSide::Home => (
            routes.match_report.step4(&space_id, &mr_id),
            "Équipe extérieure",
            false,
        ),
        TeamSide::Away => (
            routes.match_report.step5(&space_id, &mr_id),
            "Après-match",
            false,
        ),
    };
