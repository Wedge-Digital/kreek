...
mod tests {
    ...
    fn missing_at_is_rejected() {
        assert_eq!(
            Email::try_new("bagouzeexample.com").unwrap_err(),
    ...
    fn missing_domain_dot_is_rejected() {
        assert_eq!(
            Email::try_new("bagouze@example").unwrap_err(),
            EmailError::RegexViolated
        );
    }
    ...
    fn empty_local_part_is_rejected() {
        assert_eq!(
            Email::try_new("@example.com").unwrap_err(),
            EmailError::RegexViolated
        );
    }
    ...
    fn empty_domain_is_rejected() {
        assert_eq!(
            Email::try_new("bagouze@").unwrap_err(),
            EmailError::RegexViolated
        );
    }
    ...
    fn email_exceeding_255_chars_is_rejected() {
        ...
        assert_eq!(
            Email::try_new(email).unwrap_err(),
    ...
    fn whitespace_in_address_is_rejected() {
        assert_eq!(
            Email::try_new("ba gouze@example.com").unwrap_err(),
            EmailError::RegexViolated
        );
    }
}
