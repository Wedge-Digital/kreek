...
#[derive(Template)]
#[template(path = "widgets/team-selection-tester.html")]
pub struct TeamSelectionTesterTemplate {
    pub routes: AppRoutes,
    pub spaces: Vec<SpaceDefinition>,
}

impl IntoResponse for TeamSelectionTesterTemplate {
...
pub async fn get_team_selection_tester(State(state): State<AppState>) -> impl IntoResponse {
    let spaces = state
        .spaces
        .space_repository
        .find_all()
