...
fn make_token() -> Token { Token::new() }
fn make_coach_name(s: &str) -> CoachName { CoachName::try_new(s).unwrap() }

#[sqlx::test]
async fn create_inserts_token(pool: PgPool) {
    let repo = ResetTokenRepository::new(pool);

    let result = repo.create(&make_token(), &make_coach_name("Bagouze")).await;
...
async fn find_by_token_returns_token_with_correct_fields(pool: PgPool) {
    ...
    let found = repo.find_by_token(&token.to_string()).await.unwrap().unwrap();
...
async fn delete_by_token_removes_it(pool: PgPool) {
    ...
    repo.create(&token, &make_coach_name("Bagouze")).await.unwrap();
