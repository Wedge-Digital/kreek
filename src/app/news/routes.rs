...
impl Routes {
    ...
    pub fn new_article(&self, id: &str)                   -> String { path::APP_NEW_ARTICLE.replace("{space_id}", id) }
    pub fn post_article(&self, id: &str)                  -> String { path::APP_POST_ARTICLE.replace("{space_id}", id) }
    pub fn article(&self, space_id: &str, article_id: &str) -> String {
        path::APP_ARTICLE.replace("{space_id}", space_id).replace("{article_id}", article_id)
    }
    pub fn post_comment(&self, space_id: &str, article_id: &str) -> String {
        path::APP_POST_COMMENT.replace("{space_id}", space_id).replace("{article_id}", article_id)
    }
}