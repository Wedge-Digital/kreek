...
impl From<ArticleParagraph> for ParagraphViewModel {
    fn from(p: ArticleParagraph) -> Self {
        Self { title: p.title, content: p.content }
    }
}
...
const AVATAR_CLASSES: &[&str] = &[
    "avatar--blue", "avatar--green", "avatar--orange", "avatar--purple", "avatar--red",
];
...
fn format_date(dt: &time::OffsetDateTime) -> String {
    ...
    format!("{} {} {}", dt.day(), months[dt.month() as usize - 1], dt.year())
}

impl From<Comment> for CommentViewModel {
    fn from(c: Comment) -> Self {
        let initial = c.author_name
...
pub async fn get_article(
    ...
    let Some(article) = state.news.article_repository
    ...
    let comments_raw = state.news.comment_repository
        .find_by_article(&article_id)
        .await
        .unwrap_or_default();

    let word_count: usize = article.content.iter()
    ...
    let author_initial = article.author_name
        .chars().next().unwrap_or('?')
        .to_uppercase().next().unwrap_or('?')
        .to_string();

    let current_user_initial = auth_session.user.as_ref()
