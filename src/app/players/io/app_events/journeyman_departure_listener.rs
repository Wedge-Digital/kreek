//! Les deux façons dont un journalier quitte l'effectif sans avoir été embauché.
//!
//! # Deux causes, deux faits, un même état
//!
...
//!
//! Les deux posent `membership: Dismissed`, et c'est **volontairement** le même
//! état : toutes les lectures d'effectif filtrent dessus, et un journalier qui
//! part est un journalier qui part. Ce qui diffère est l'histoire, et l'event
//! store la garde — c'est aussi pourquoi ni l'un ni l'autre n'est un
//! `PlayerDismissed`, qui raconterait une décision de renvoi jamais prise.
//!
...
//! le précèdent. Quand ce listener s'exécute, les journaliers gardés sont déjà
//! `Active`, et il ne trouve que ceux qui restent.
//!
//! L'ordre est donc tenu par la structure du lot, pas par une précaution ici.
//! `un_journalier_recrute_survit_a_la_sortie_de_phase` échoue si quelqu'un le
//! change — ce qui compilerait parfaitement.
//!
        ...
        loop {
            match rx.recv().await {
                Ok(envelope) => {
                    ...
                    if let Ok(TeamsAppEvent::RecruitmentPhaseValidated { team_id, .. }) =
