...
fn action_line_vm(action: &MatchHistoryAction) -> MatchActionLineVm {
    ...
    MatchActionLineVm { icon, label, spp: action.spp_earned }
}
...
async fn load_events(state: &AppState, player: &Player) -> Result<Vec<crate::app::players::domain::events::PlayerDomainEvent>, Response> {
    state.players.repository.find_events_by_id(&player.id).await.map_err(|e| {
...
struct SppBreakdown { earned: u32, spent: u32, reserve: u32, percent: u8 }
...
fn build_vm(
    ...
    let match_history = build_match_history(events).into_iter().map(match_history_card_vm).collect();
    ...
    PlayerDetailVm {
        ...
        ma: stats.ma, st: stats.st, ag: stats.ag, pa: stats.pa, av: stats.av,
        base_skills, acquired_skills,
        value_formatted: format!("{} Po", format_thousands(player.value.0)),
        spp_earned: spp.earned, spp_spent: spp.spent, spp_reserve: spp.reserve, spp_percent: spp.percent,
...
mod tests {
    ...
    fn action_line_vm_maps_touchdown_icon_and_label() {
        let action = MatchHistoryAction { kind: MatchHistoryActionKind::Touchdown, spp_earned: Some(3) };
    ...
    fn action_line_vm_foul_has_no_spp() {
        let action = MatchHistoryAction { kind: MatchHistoryActionKind::Foul, spp_earned: None };
        let vm = action_line_vm(&action);
        assert_eq!(vm.icon, "🟨");
    ...
    fn match_history_card_vm_derives_victory_result() {
        let entry = MatchHistoryEntry {
            match_report_id: "mr1".into(), round_label: "Journée 1".into(),
            opponent_team_name: "Bone Crushers".into(), team_score: 3, opponent_score: 1,
            actions: vec![
                crate::app::players::use_cases::match_history_service::MatchHistoryAction { kind: MatchHistoryActionKind::Touchdown, spp_earned: Some(3) },
                crate::app::players::use_cases::match_history_service::MatchHistoryAction { kind: MatchHistoryActionKind::Foul, spp_earned: None },
            ],
        };
    ...
    fn match_history_card_vm_derives_defeat_and_draw() {
        let defeat = MatchHistoryEntry {
            match_report_id: "mr1".into(), round_label: "J1".into(), opponent_team_name: "X".into(),
            team_score: 0, opponent_score: 2, actions: vec![],
        };
        assert_eq!(match_history_card_vm(defeat).result_label, "Défaite");

        let draw = MatchHistoryEntry {
            match_report_id: "mr2".into(), round_label: "J2".into(), opponent_team_name: "X".into(),
