...
mod tests {
    ...
    impl IPlayerRepository for FakePlayerRepo {
        async fn append(&self, _: &PlayerId, _: &TeamId, event: &PlayerDomainEvent, _version: i32) -> Result<(), RepositoryError> {
    ...
    fn created_event() -> PlayerDomainEvent {
        PlayerDomainEvent::PlayerCreated {
            ...
            jersey: None, base_skills: vec![], starting_spp: Spp(100), starting_value: ValueKpo(100_000),
        }
    }
    ...
    fn skill_and_stat_purchases_share_the_same_level_counter() {
        ...
        let skill_event = player.purchase_skill(
            SkillId::try_new("block").unwrap(), SkillName::try_new("Bloc").unwrap(),
            "type-general".into(), crate::app::players::domain::player::AcquisitionMode::Chosen,
            SppCost::try_new(1).unwrap(), ValueKpo(0),
        ).unwrap();
