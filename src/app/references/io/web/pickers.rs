    ...
    let mut items: Vec<RosterPickerItemWithTier> = rules
        ...
        .flat_map(|(i, tier)| {
            ...
            tier.rosters.iter().filter_map(move |uid| {
                repo.find_team_by_uid(uid)
                    .map(|t| RosterPickerItemWithTier {
                        uid: t.uid.clone(),
                        name: t.name.clone(),
...
pub fn build_player_positions(
    team: &Team,
    repo: &dyn IReferenceRepository,
) -> Vec<PlayerPositionVm> {
...
pub fn build_roster_items(repo: &dyn IReferenceRepository) -> Vec<RosterPickerItem> {
    let mut items: Vec<RosterPickerItem> = repo
        .list_teams()
        .iter()
        .map(|t| RosterPickerItem {
            uid: t.uid.clone(),
            name: t.name.clone(),
        })
        .collect();
