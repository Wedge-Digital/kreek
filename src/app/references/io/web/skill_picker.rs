...
pub async fn skill_picker(
    ...
    let pricing_vm = PricingVm {
        chosen: ChosenCostVm {
            primary: pricing.chosen.primary,
            secondary: pricing.chosen.secondary,
        },
    ...
    let mode = if params.mode == "random" {
        "random"
    } else {
        "chosen"
    };
    ...
    let active_filters: Vec<&str> = params
        .filters
        .split(',')
        .filter(|s| !s.is_empty())
        .collect();
