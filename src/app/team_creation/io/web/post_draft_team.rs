...
pub async fn post_draft_team(
    ...
    let team_name = match TeamName::try_new(form.team_name) {
        Ok(n)  => n,
        Err(_) => return error_response("Le nom d'équipe est invalide (1–50 caractères alphanumériques)."),
    };

    let coach_id_str = form.coach_id.filter(|s| !s.is_empty())
    ...
    let created_by = UserId::try_new(&user_id_str)
        .unwrap_or_else(|_| user.id.clone());
    ...
    if let Err(e) = state.team_creation.team_repository
    ...
    Response::builder()
        .header("HX-Redirect", format!("/app/{}/team/{}/build", space_id_raw, team.get_id()))
        .body(Body::empty())
        .unwrap()
        .into_response()
}
