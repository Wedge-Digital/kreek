...
use crate::app::spaces::uses_cases::join_spaces::{JoinSpacesCommand, execute};
...
pub async fn join_spaces(
    ...
    let space_ids: Vec<SpaceId> = payload.space_ids
        .iter()
    ...
    let cmd = JoinSpacesCommand { coach_id: user.id, space_ids };

    if execute(cmd, &*state.spaces.space_repository, &state.spaces.event_bus).await.is_err() {
