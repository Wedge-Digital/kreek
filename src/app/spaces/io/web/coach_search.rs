...
pub async fn get_coach_search_widget(Path(space_id): Path<String>) -> impl IntoResponse {
...
mod tests {
    ...
    fn build_router(users: Vec<SpaceUser>) -> Router {
        ...
        let auth_layer = AuthManagerLayerBuilder::new(
            AuthBackend::new(
                user_repo.clone() as Arc<dyn crate::app::auth::ports::IUserRepository>,
                false,
            ),
            session_layer,
        )
    ...
    async fn query_filters_by_name_case_insensitive() {
        ...
        let users = vec![
            make_user("Bagouze"),
            make_user("LeTacticien"),
            make_user("Colonel Castor"),
        ];
        let app = build_router(users);
