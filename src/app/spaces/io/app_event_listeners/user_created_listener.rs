                ...
                Ok(event) => {
                    ...
                    tokio::spawn(async move {
                        let sub = match serde_json::from_value::<AuthAppEvent>(event.payload) {
                            Ok(s)  => s,
                            Err(e) => { tracing::error!("user_created: payload invalide: {e}"); return; }
                        };
                        let AuthAppEvent::AccountCreated { user_id, user_name, email, .. } = sub else { return; };
                        if let Err(e) = repo.add_user(&User { id: user_id, name: user_name, email, icon: None }).await {
...
mod tests {
    ...
    struct FakeUserCacheRepository { added: Mutex<Vec<User>> }
    ...
    async fn account_created_app_event_adds_user_to_cache() {
        ...
        user_created_listener(&bus, Arc::clone(&repo) as Arc<dyn ISpaceUserCacheRepository>);
